...
- **AMD ADL2 legacy Overdrive telemetry is completely non-functional on this GPU, confirmed by direct return-code diagnostics.** `ADL2_Adapter_NumberOfAdapters_Get` enumerates 10 logical adapter slots; `ADL2_Overdrive5_Temperature_Get` returns `-1` (`ADL_ERR`) and `ADL2_OverdriveN_Temperature_Get` returns `-8` (`ADL_ERR_NOT_SUPPORTED`) on **every** one of them. This is not an adapter-selection bug — both the legacy (OD5) and the newer-but-still-legacy (OverdriveN) sensor APIs are simply unsupported by this driver/GPU generation. Hardened `AmdAdlContext::init()`'s adapter-selection loop to accept either OD5 or OverdriveN success (a real, defensible improvement for older transitional hardware where one API works and the other doesn't), but confirmed this does **not** restore telemetry on the RX 9070 XT — both still fail. Net effect: AMD discrete GPU temperature, usage, clocks, fan RPM, and power draw on this class of card fall back to the (weaker) WMI path instead of ADL, unlike the NVML path which is unaffected. A real fix needs AMD's current ADLX SDK (a substantially different, COM-style C++ interface vs. the existing flat-function ADL2 FFI) — **a larger migration task, not attempted in this pass.** Tracked as a Now-priority backlog item in `docs/radium-improvement-audit.md`.
