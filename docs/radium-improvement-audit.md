...
- **AMD ADL2's entire legacy Overdrive telemetry surface (OD5 and OverdriveN) is non-functional on current-generation RDNA3/RDNA4 cards**, confirmed by raw return-code diagnostics on a Radeon RX 9070 XT: `ADL2_Overdrive5_Temperature_Get` returns `-1` and `ADL2_OverdriveN_Temperature_Get` returns `-8` (`ADL_ERR_NOT_SUPPORTED`) on all 10 enumerated adapter slots. The adapter-selection loop in `amd_provider.rs` was hardened on 2026-06-27 to accept either OD5 or OverdriveN success (helps older transitional hardware), but that alone does not restore telemetry on RDNA3/4 — `amd_provider.rs` needs an ADLX SDK code path (a substantially different COM-style interface, not a drop-in FFI change) before AMD discrete GPU temp/usage/clock/fan/power telemetry can be trusted on new hardware. Until that migration happens, these cards silently get WMI-only quality instead of ADL quality.
