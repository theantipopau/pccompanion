...
- **Implemented and validated `src-tauri/src/adlx_provider.rs`**, a new AMD GPU telemetry provider using AMD's current ADLX SDK (`amdadlx64.dll`), replacing the legacy ADL2 path as the preferred AMD provider in `hardware.rs`'s priority chain (NVML -> ADLX -> legacy ADL2 -> IGCL -> WMI). ADLX exposes C++ virtual interfaces rather than flat C exports; the vtable struct layouts were transcribed verbatim from AMD's official public SDK headers (`https://github.com/GPUOpen-LibrariesAndSDKs/ADLX`, downloaded locally and read directly rather than trusting an AI-summarized fetch, since exact vtable slot order is safety-critical) rather than reverse engineered.
- Validated on real hardware: correct discrete-GPU selection (this host enumerates both an integrated/chipset `AMD Radeon(TM) Graphics` adapter and the discrete `AMD Radeon RX 9070 XT`; an early version of the selection logic had a real bug that picked the wrong one — fixed and re-verified), real temperature (~29-34°C idle), VRAM-used (~1.9 GB idle, cross-checked against the registry-corrected total VRAM), and usage (0-3% idle) readings stable across 5 repeated polls. Clock-speed and power readings are gated per-metric on `IADLXGPUMetricsSupport::IsSupportedX` flags, since the driver was observed returning `ADLX_OK` with a meaningless value for at least one genuinely-unsupported metric (power) on this card rather than failing the call.
- Hit and root-caused one real `STATUS_ACCESS_VIOLATION` crash during this work: it came from calling `ADLXInitialize` a second time in-process after a prior session had already called `ADLXTerminate`, an artifact of an aggressive re-init test pattern, not the vtable layout. Production usage (single init held for the monitoring thread's lifetime) does not exercise this path; re-verified stable with a single-init + repeated-poll test matching real usage.
- GPU usage/power were only exercised at idle in this pass (test machine was idle the whole session); a load-test pass is still recommended before fully trusting those two specific fields under heavy gaming/rendering workloads.
...
- **Confirmed AMD ADL2 legacy Overdrive telemetry is fully non-functional on this RDNA4 card, not an adapter-selection bug.** Added a temporary diagnostic that dumped raw ADL2 return codes: `ADL2_Adapter_NumberOfAdapters_Get` enumerates 10 logical adapter slots; `ADL2_Overdrive5_Temperature_Get` returns `-1` and `ADL2_OverdriveN_Temperature_Get` returns `-8` (`ADL_ERR_NOT_SUPPORTED`) on every single one. Hardened the adapter-selection loop in `amd_provider.rs` to accept either OD5 or OverdriveN success (a real, low-risk improvement for older transitional hardware), but confirmed it does not restore telemetry on this card — both APIs are simply unsupported by this driver/GPU generation. AMD discrete GPU temperature/usage/clocks/fan/power on current-generation cards still fall back to the weaker WMI path. A real fix needs migration to AMD's ADLX SDK (a substantially different COM-style interface, not a drop-in FFI change) — added as a larger, separate task to Priority 4 below, not attempted in this pass.
...
- **Fixed 2026-06-27**: AMD ADL2's entire legacy Overdrive telemetry surface (OD5 and OverdriveN) is non-functional on current-generation RDNA3/RDNA4 cards, confirmed by raw return-code diagnostics on a Radeon RX 9070 XT: `ADL2_Overdrive5_Temperature_Get` returns `-1` and `ADL2_OverdriveN_Temperature_Get` returns `-8` (`ADL_ERR_NOT_SUPPORTED`) on all 10 enumerated adapter slots. Rather than patching the legacy path further, added `adlx_provider.rs` using AMD's current ADLX SDK and wired it ahead of legacy ADL2 in `hardware.rs`'s provider priority. Validated real temperature/VRAM/usage on this hardware; see the 2026-06-27 changelog entry above for full detail.
...
- AMD ADLX migration done 2026-06-27 (`adlx_provider.rs`); real temp/VRAM/usage confirmed on a Radeon RX 9070 XT. Remaining: validate GPU usage/power under real load (only exercised at idle so far), and revisit `query_driver_version()` parity (ADLX doesn't currently expose a driver-version getter the way legacy ADL2 did).
