...
export function RegistryCleanerPage() {
  ...
  const hasScanned = issues.length > 0;
  ...
  return (
    <div className="page desktop-page">
      <PageHeader
        ...
        description="Reviews orphaned startup entries, uninstall leftovers, broken file associations, and dead application paths. A backup is written to Documents before any change."
        action={
      ...
      <div className="reg-how-it-works">
        <RegStep num={1} title="Scan" description="Issues are detected automatically on page load." done={hasScanned} />
        <RegStep num={2} title="Review" description="Read each entry. Tick only what you recognise as safe." done={hasSelected} />
        <RegStep num={3} title="Backup" description="All selected keys are exported to Documents first." done={hasBackup} />
        <RegStep num={4} title="Clean" description="Backup &amp; clean removes only safe-flagged selections." done={false} />
      </div>

      <div className="cleaner-shell">
        <Panel className="cleaner-summary">
          <SummaryItem icon={ScanLine} label="Issues found" value={issues.length.toString()} tone="cyan" />
          <SummaryItem icon={ShieldCheck} label="Safe to remove" value={safeSelected.length.toString()} tone="green" />
          <SummaryItem icon={Archive} label="Backup" value={backup ? 'Ready' : 'Required'} tone={backup ? 'green' : 'amber'} />
        </Panel>
        ...
        <Panel className="registry-categories">
          ...
          <div className="reg-safety-legend">
            <strong>safe</strong> = low-risk orphan &nbsp;·&nbsp; <strong>review</strong> = inspect before removing
          </div>
        </Panel>
        ...
        <Panel className="manager-table registry-results">
          <div className="panel-heading">
            <div>
              <span className="eyebrow">Review carefully before selecting</span>
              <h2>{activeCategory === 'All' ? 'All findings' : activeCategory}</h2>
            </div>
            <FileWarning size={19} />
          </div>
          {visibleIssues.length === 0 ? (
            <p className="reg-empty">No issues in this category.</p>
          ) : (
                ...
                <label className="registry-row" key={issue.id}>
                  ...
                  <div className="registry-row-body">
                    <div className="cleanup-item-title">
                      <strong>{issue.valueName || issue.category}</strong>
                    ...
                    <code>{issue.hive}\{issue.keyPath}{issue.valueName ? `\\${issue.valueName}` : ''}</code>
                  </div>
                </label>
              ))}
            </div>
        ...
        <Panel className="manager-inspector">
          <div className="panel-heading">
            <div>
              <span className="eyebrow">Output log</span>
              <h2>Actions</h2>
            </div>
            <Archive size={19} />
          </div>
          ...
          <div className="reg-guarantees">
            <div className="reg-guarantee-item"><CheckCircle2 size={15} /><span>Registry keys are exported before any removal</span></div>
            <div className="reg-guarantee-item"><CheckCircle2 size={15} /><span>High-risk entries are left unticked by default</span></div>
            <div className="reg-guarantee-item"><CheckCircle2 size={15} /><span>Backup path is logged before each clean operation</span></div>
          </div>
        </Panel>
      </div>
    </div>
...
function RegStep({ num, title, description, done }: { num: number; title: string; description: string; done: boolean }) {
  return (
    <div className={done ? 'reg-step done' : 'reg-step'}>
      <span className="reg-step-num">{num}</span>
      <div>
        <strong>{title}</strong>
...
function SummaryItem({ icon: Icon, label, value, tone }: { icon: LucideIcon; label: string; value: string; tone?: string }) {
  return (
